...
export default function OutputPanel({ outputText, onAddSale, clientCelular }: { outputText: string, onAddSale: () => void, clientCelular?: string }) {
  ...
  return (
    <div className="panel always" style={{ marginTop: '1.25rem' }}>
      ...
      <div className="actions" style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginTop: '0.75rem' }}>
        <button className="btn btn-primary" onClick={handleCopy} style={{ flex: 1, minWidth: '130px' }}>
          {copied ? <><Check size={16} /> ¡Copiado!</> : <><Copy size={16} /> Copiar</>}
        </button>
        <button
          onClick={handleWhatsApp}
          style={{
            flex: 1, minWidth: '130px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem',
        ...
        <button className="btn btn-primary" onClick={handleAddSale} style={{ flex: 1, minWidth: '130px', background: added ? 'linear-gradient(135deg, #16a34a, #15803d)' : undefined }}>
